...
int main(int argc, char* argv[]) {
    ...
    */
    double global_elapsed = 0.0, global_initial = 0.0, global_final = 0.0;
    global_initial = omp_get_wtime();
    for (int i = n - 1; i >= 0; i--) {
        x[i] = b[i];
        int reducer = x[i];
#pragma omp parallel for num_threads(numThreads) \ 
        schedule(runtime) reduction(+: reducer)
        for (int j = i+1; j < n; j++) {
            reducer += (-1 * A[i][j]*x[j]); 
        }
        x[i] = reducer;
    ...
    global_final = omp_get_wtime(); 
    /*for (int i = 0; i < n; i++)
      std::cout << "x" << i << " : " << x[i] << std::endl;
    */
    std::cout << "total time: " << global_final - global_initial << std::endl;
